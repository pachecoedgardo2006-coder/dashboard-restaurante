...
export function TarjetaPedido({ pedido, onCambiarEstado }) {
    ...
    card.className = `p-4 rounded-xl border flex flex-col justify-between h-full transition-all shadow-sm hover:shadow-md ${
    ...
    card.innerHTML = `
        <div class="space-y-2">
            <div class="flex justify-between items-center gap-2">
                <span class="text-[10px] font-mono px-2 py-0.5 rounded font-bold bg-slate-100 text-slate-500 border border-slate-200">
                    #ID ${pedido.id}
                </span>
            ...
            <div class="min-w-0">
                <h3 class="font-black text-sm text-slate-900 truncate uppercase tracking-tight">${pedido.cliente_nombre}</h3>
