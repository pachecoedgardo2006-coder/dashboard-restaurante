...
export function FilaHistorial({ pedido }) {
    ...
    const estadoBadge = pedido.estado === 'Entregado' 
        ? `<span class="px-3 py-1 text-[10px] font-black uppercase bg-emerald-50 border border-emerald-200 text-emerald-600 tracking-wider rounded">${t('estados.Entregado')}</span>`
        : `<span class="px-3 py-1 text-[10px] font-black uppercase bg-red-50 border border-red-200 text-red-600 tracking-wider rounded">${t('estados.Cancelado')}</span>`;
    ...
    tr.innerHTML = `
        ...
        <td class="p-4 text-xs text-slate-600 font-bold uppercase tracking-wide">
            <span class="text-slate-400">${t('historial.torre')}</span> ${pedido.torre_bloque} 
            <span class="text-slate-400 ml-2">${t('historial.apt')}</span> ${pedido.apartamento}
        </td>
        ...
        <td class="p-4 text-center text-xs uppercase font-bold tracking-wide">
            <span class="text-slate-600 block">${t(`pagos.${pedido.tipo_pago}`)}</span>
            ${pedido.tipo_pago === 'Efectivo' ? `<span class="text-[10px] text-slate-400 font-mono tracking-normal block mt-0.5">${t('historial.vuelto')} $${pedido.cambio.toLocaleString()}</span>` : ''}
        </td>
