...
export async function renderHistorial() {
    ...
    container.innerHTML = `
        <header class="border-b border-slate-900 pb-4">
            <h1 class="text-3xl font-black tracking-tighter text-white uppercase">${t('historial.titulo')}</h1>
            <p class="text-xs text-slate-400 mt-1 uppercase tracking-wider font-semibold">${t('historial.subtitulo')}</p>
        </header>
        ...
        <section class="bg-slate-950 border border-slate-900 rounded-none overflow-hidden shadow-2xl">
            <div class="p-4 bg-slate-900/60 border-b border-slate-900 flex flex-col xl:flex-row gap-4 justify-between items-center">
                <h2 class="text-xs font-black text-mostaza-caliente uppercase tracking-widest flex items-center gap-2"> ${t('historial.ordenesArchivadas')}
                </h2>
                ...
                <form id="form-filtro-fechas" class="flex flex-wrap items-center gap-3 bg-slate-950 p-2 border border-slate-900 rounded-none">
                    <div class="flex items-center gap-2">
                        <label class="text-[10px] uppercase tracking-widest font-black text-slate-500">${t('historial.desde')}</label>
                    ...
                    <div class="flex items-center gap-2">
                        <label class="text-[10px] uppercase tracking-widest font-black text-slate-500">${t('historial.hasta')}</label>
                ...
                <span id="contador-historial" class="text-xs font-mono bg-slate-900 px-3 py-1 text-mostaza-caliente border border-slate-800 font-bold">${t('historial.pedidoCount', { count: 0, plural: 'S' })}</span>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm border-collapse">
                    ...
                    <tbody id="tabla-cuerpo" class="divide-y divide-slate-900">
                        <tr><td colspan="7" class="text-center py-12 text-slate-600 text-xs uppercase font-bold tracking-wider">${t('historial.cargando')}</td></tr>
                    </tbody>
                </table>
            </div>
        </section>
    ...
    async function cargarHistorial(desde = '', hasta = '') {
        ...
        try {
            ...
            if (archivados.length === 0) {
                tbody.innerHTML = `<tr><td colspan="7" class="text-center py-12 text-slate-500 text-xs uppercase font-bold tracking-wider">${t('historial.sinRegistros')}</td></tr>`;
                return;
            }
