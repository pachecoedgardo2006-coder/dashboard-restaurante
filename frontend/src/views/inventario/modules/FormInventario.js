...
export function FormInventario({ onGuardar, onCancelarEdicion }) {
    ...
    container.innerHTML = `
        <div>
            <h2 id="form-titulo" class="text-sm font-black text-rojo-fuego uppercase tracking-widest">${t('inventario.form.agregarTitulo')}</h2>
            <p class="text-[11px] text-slate-500 uppercase tracking-wider font-bold mt-1">${t('inventario.form.agregarSubtitulo')}</p>
        </div>
        
        <form id="form-producto-inv" class="space-y-4" autocomplete="off">
            <input type="hidden" id="edit-id" value="">
            <div>
                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">${t('inventario.form.nombreCompleto')}</label>
            ...
            <div>
                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">${t('inventario.form.descripcionCorta')}</label>
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">${t('inventario.form.precio')}</label>
                ...
                <div>
                    <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">${t('inventario.form.stockInicial')}</label>
